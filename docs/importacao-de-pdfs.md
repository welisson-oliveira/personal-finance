...
- **Aprende regras das classificações do preview:** para cada linha com `learn=true` (o usuário classificou tipo/grupo/direção/categoria no preview), `learnRule` faz upsert de uma `MerchantRule` USER (pula "Pagamento de fatura") — mesma aprendizagem que o edit da lista faz, para as próximas importações virem classificadas.
