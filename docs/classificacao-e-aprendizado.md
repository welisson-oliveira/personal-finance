...
| ------------------------------------------------ | ------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| `merchant_rules` / `MerchantRule`                | `normalizedName` (+ `user` nullable) | **override** de um estabelecimento: categoria + `expenseType` (budget group) **+ `type` + `ignored` + `investmentDirection`** (V14) **+ `reimbursement`** (V21). `user=null` = regra global do sistema. `createdBy` SYSTEM/USER, `confidence` 0–100 |
...
- **`ClassificationResult`** — valor imutável (categoryId/name, subcategory, expenseType, confidence, **type, ignored, investmentDirection, reimbursement**). `isKnown()` = confidence > 0; `isAutoClassifiable()` = confidence ≥ 80.
...
- **Ao editar uma transação** (qualquer tipo): faz upsert de uma `MerchantRule` do usuário com `confidence=100`, `createdBy=USER`, gravando **`type`, `ignored`, categoria, `expenseType`, `investmentDirection` e `reimbursement`**. Assim a próxima importação do mesmo estabelecimento vem já classificada — inclusive corrigindo heurísticas (ex.: "esta transferência Open Banking é meu salário, não é transferência própria, não ignore") e marcando reembolsos recorrentes (ex.: "o rateio da conta de luz que o colega me devolve todo mês é reembolso da faixa Essencial").
- **Aplicação na importação:** para **cada** transação, se existe uma regra **do usuário** (`findUserOverride`), ela é aplicada **antes** de qualquer heurística (own-transfer, investimento do parser, regra global de despesa) — `applyUserOverride` seta tipo/ignored/categoria/grupo/direção e `needsReview=false`. Para uma regra de **INCOME reembolso**, o override marca `reimbursement=true` e carrega a categoria + `budget_group` (a faixa que o reembolso abate); INCOME normal zera esses campos.
...
- propagar tipo/categoria/budgetGroup/investmentDirection/ignored/reimbursement ao editar uma transação (`TransactionService.propagateClassification`);
